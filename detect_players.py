...
def detect_players_with_roboflow(image_path):
    ...
    for prediction in predictions['predictions']:
        ...
        cv2.putText(image, f'{prediction['class']} {str(round(prediction['confidence'], 2))}', (int(start_x - width), int(start_y - height) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    if rim_y and ball_y:
...
detect_players_with_roboflow('data/frame50.jpg')